...
void MarkerSet::set_surface_marker(const Variables& var, const double smallest_size, const int mattype, double_vec& edvacc, int_vec2D& elemmarkers)
{
#ifdef USE_NPROF
    nvtxRangePushA(__FUNCTION__);
#endif
...
//    #pragma omp parallel for default(none) shared(var,mattype,edvacc,elemmarkers,src_locs,std::cout)
    ...
    for (size_t i=0; i<(*var.surfinfo.top_facet_elems).size(); i++) {
        ...
        double rv = edvacc[e] / (*var.volume)[e];
        if (rv < 0.05) continue;
